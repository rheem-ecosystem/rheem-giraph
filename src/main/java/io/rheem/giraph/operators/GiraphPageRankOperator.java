...
import io.rheem.giraph.Algorithm.PageRankAlgorithm;
import io.rheem.giraph.Algorithm.PageRankParameters;
...
public class GiraphPageRankOperator extends PageRankOperator implements GiraphExecutionOperator {
    ...
    private Stream<Tuple2<Long, Float>> createStream(String path) {
        return io.rheem.core.util.fs.FileUtils.streamLines(path).map(line -> {
